...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(operator_eval)
        operand_eval = rest.map(lambda each_operand: scheme_eval(each_operand, env))
        return scheme_apply(operator_eval, operand_eval, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals == nil:
            ...
            return child_frame.make_child_frame(formals.rest, vals.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        evaluate = scheme_eval(expressions.rest.first, env)
        env.define(target, evaluate)
        return target
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    expression_eval = scheme_eval(expressions.first, env)
    if is_false_primitive(expression_eval):
        return False
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    expression_eval = scheme_eval(expressions.first, env)
    if is_true_primitive(expression_eval):
        return expression_eval
    elif expressions.rest is nil:
        return False
    else:
